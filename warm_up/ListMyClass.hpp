...
#include "Person.hpp"
#include "A-Sort-Tester.cpp"
// Comparison function, used later.
bool
MyClass_less(const MyClass &p1, const MyClass &p2) {
    return strcmp(p1->name, p2->name) < 0;
}

#ifndef LISTMyClass_HPP
#define LISTMyClass_HPP
...
struct List_MyClass_Iterator
{
  Link    *link;
  MyClass (*deref)(List_MyClass_Iterator*);
  void    (*inc)  (List_MyClass_Iterator*);
  void    (*dec)  (List_MyClass_Iterator*);
  //void    (*erase)  (List_MyClass_Iterator*);
};

struct List_MyClass  {
  Link head; 
  int  count; // number of nodes
  char type[(int)sizeof("List_MyClass")];

  List_MyClass_Iterator (*begin)     (List_MyClass*);
  List_MyClass_Iterator (*end)       (List_MyClass*);
  List_MyClass_Iterator (*push_front)(List_MyClass*, MyClass);
  List_MyClass_Iterator (*push_back) (List_MyClass*, MyClass);
  void (*sort)(List_MyClass*);
  bool (*sort_style)      (const MyClass &, const MyClass &);  
  //implementation of sort lives in test.cpp, where this 
  //struct will be part of a huge macro
  void (*erase)(List_MyClass*, List_MyClass_Iterator);
  void (*delet)(List_MyClass*);
};
...
List_MyClass_Iterator push_front(List_MyClass* list, MyClass p);
List_MyClass_Iterator push_back(List_MyClass* list, MyClass p);
void inc(List_MyClass_Iterator *it);
void dec(List_MyClass_Iterator *it);
//void List_MyClass_Iterator_delet(List_MyClass_Iterator *it);
List_MyClass_Iterator end(List_MyClass* list);
List_MyClass_Iterator begin(List_MyClass* list);
MyClass deref(List_MyClass_Iterator* it);
List_MyClass_Iterator List_MyClass_Iterator_new(Link* link);
void erase(List_MyClass* list, List_MyClass_Iterator it);
void List_MyClass_delet(List_MyClass* list);


// container constructor for MyClass, MyClass, int, & Stress
// substitute @@@@ for those types of objects via the macro
List_MyClass *List_MyClass_new(bool (*sort_style)(const MyClass &p1, const MyClass &p2))
{
	List_MyClass *list = (List_MyClass *)malloc(sizeof(List_MyClass));
  list->head.next = &(list->head); 
  list->head.prev = &(list->head);
  List_MyClass_Iterator it = begin(list);
	list->count = 0;
  strcpy(list->type, "List_MyClass");//list->type = "List_MyClass";
  list->begin = begin;
  list->end = end;
  list->sort = &sort;
  list->erase = erase;  
  list->delet = List_MyClass_delet;
  list->sort_style = MyClass_less; 
...
List_MyClass_Iterator push_front(List_MyClass* list, MyClass p) {
  ...
  return List_MyClass_Iterator_new( &(n->link) );
}
...
List_MyClass_Iterator List_MyClass_Iterator_new(Link* link){
  ...
  it.deref = &deref;
  it.dec = &dec;
  //it.erase = &List_MyClass_Iterator_delet;
  return it;
}
...
bool 
List_MyClass_Iterator_equal( List_MyClass_Iterator it1, 
                               List_MyClass_Iterator it2)
{ 
...
//end
List_MyClass_Iterator end(List_MyClass* list) {
  return List_MyClass_Iterator_new(&(list->head));
}

//begin
List_MyClass_Iterator begin(List_MyClass* list) {
  return List_MyClass_Iterator_new(list->head.next);
}
/* test.cpp provides a function to print
void print(List_MyClass* list)
{                          int x = 1;
...
struct Link* SortedMerge(struct Link* a, struct Link* b)
{
  ...
  if ( 
        MyClass_less(         // returns true if alphabetical order
                        ((Node*)(a))->p, // e.g. jane
                        ((Node*)(b))->p  // e.g. mike
                      )
     )
