...
// DUMMY EXAMPLE of a macro to declare and define stuff
//         .........BEFORE main()............
#define List_DEFINE(argument) struct arf_##argument \
                       {                            \
                         int two;                   \
                         double golden;             \
                         char z;                    \
...
struct Node {
  Link link;
  PersonPtr p;
  int value; // unnecessary; just here for testing
};
...
struct List_PersonPtr  {
  ...
  void (*push_front)(List_PersonPtr*, PersonPtr);
  void (*push_back) (List_PersonPtr*, PersonPtr);
  bool (*sort)      (const PersonPtr, const PersonPtr);  
...
//extern bool PersonPtr_less(const PersonPtr, const PersonPtr);

List_PersonPtr *List_PersonPtr_new(bool PersonPtr_less(const PersonPtr, const PersonPtr));
void insert_before(Link *pos, Node *n);
//void remove_node(Node *n);

//Doubly-Linked, Circular Lists with Sentinel
void push_front(List_PersonPtr* list, PersonPtr p);
void push_back(List_PersonPtr* list, PersonPtr p);
...
List_PersonPtr_Iterator List_PersonPtr_Iterator_new(Link* link);
...
{  
  ...
  List_PersonPtr_Iterator it = begin(list);
	list->count = 0;
  strcpy(list->type, "List_PersonPtr");//list->type = "List_PersonPtr";
  list->begin = begin;
  list->end = end;
  list->sort_style = PersonPtr_less; //can we make arg input more generic? e.g. able to take PersonPtr_greater?
...
void print(List_PersonPtr* list)
{                          int x = 1;
...
print(List_PersonPtr *list) {
    ...
    while (!List_PersonPtr_Iterator_equal(it, list->end(list))) {
      printf("%p, %p\n", it.link, (list->end(list)).link);
                                                                  printf("---------------------------------------------->%d\n", x++);
        printf("    %s\n", it.deref(&it)->name);//%%%%%%%%%%%%%%%%%%%fails here
                                                                  printf("---------------------------------------------->%d\n", x++);
        it.inc(&it);
    }
    printf("---- Print end\n");
...
main(int argc, char *argv[]) {
    ...
    */
    ...
    Person *p1 = 0;
    {
        ...
        //     Name: <name>
        //     Gender: <gender>
        Person_print(p1); // helper function in this file
...
// mucho unit testing here


//added for testing; leave in as a unit test
  Node *b = (Node*)malloc(sizeof(Node));
  b->value = 200;
  b->p = p4;
  printf("list starts with: %d\n", b->value);
  printf("%s is a %s\n", b->p->name, b->p->type_name);
  free(b);
...
// format 'p' is for pointer (some kind of long)
printf("\n     printf:   &(list->head): %p \n", &(list->head));
//printf(  "     printf:      list->head: %p \n", list->head); // accessing first value of list->head, which is actually 'next's value, which points to the next node 
printf(  "     printf: list->head.next: %p \n", list->head.next);
printf(  "     printf: list->head.prev: %p \n\n", list->head.prev);
//head should point to both the first and last node in the list. 
//In this way, you can access the end of list in O(1) time also.

        // Insert people into the List.
        list->push_front(list, p1);
printf("do: list->push_front(list, p1);\n");
...
printf(  "list->head.next->next: %p \n", list->head.next->next);
printf(  "list->head.next->prev: %p \n", list->head.next->prev);
printf(  "list->head.next->next->next: %p \n", list->head.next->next->next);
printf(  "list->head.next->next->prev: %p \n\n", list->head.next->next->prev);

        list->push_front(list, p4);
printf("do: list->push_front(list, p4);\n");
printf("\n  &(list->head): %p \n", &(list->head));
//printf(  "     list->head: %p \n", list->head); // accessing first value of list->head, which is actually 'next's value, which points to the next node
printf(  "list->head.next: %p \n", list->head.next);
printf(  "list->head.prev: %p \n", list->head.prev);
...
printf(  "list->head.next->next->next: %p \n", list->head.next->next->next);
printf(  "list->head.next->next->prev: %p \n", list->head.next->next->prev);
printf(  "list->head.next->next->next->next: %p \n", list->head.next->next->next->next);
printf(  "list->head.next->next->next->prev: %p \n\n", list->head.next->next->next->prev);


printf(">>>>>>>>> %s\n", ((Node*)(list->head.next))->p->name );
printf("VERY GOOD >>>>>>>>> %s\n", ((Node*)(list->head.next->next))->p->name );
        print(list);
        list->push_front(list, p3);
printf(">>>>>>>>> %s\n", ((Node*)(list->head.next))->p->name );
printf("VERY GOOD >>>>>>>>> %s\n", ((Node*)(list->head.next->next))->p->name );
printf("VERY VERY GOOD >>>>>>>>> %s\n", ((Node*)(list->head.next->next->next))->p->name );
        print(list);
        list->push_front(list, p4);
printf(">>>>>>>>> %s\n", ((Node*)(list->head.next))->p->name );
printf("VERY GOOD >>>>>>>>> %s\n", ((Node*)(list->head.next->next))->p->name );
printf("VERY VERY GOOD >>>>>>>>> %s\n", ((Node*)(list->head.next->next->next))->p->name );
printf("VERY VERY VERY GOOD >>>>>>>>> %s\n", ((Node*)(list->head.next->next->next->next))->p->name );
/*
Student_deleet(p1);
Student_deleet(p2);
Student_deleet(p3);
...
// this block has diagnostics for the 10 or so lines of code that follow 
#endif
