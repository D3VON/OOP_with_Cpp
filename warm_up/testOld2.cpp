...
struct List_PersonPtr_Iterator
{
  ...
  //void    (*erase)  (List_PersonPtr_Iterator*);
};
...
bool PersonPtr_less(const PersonPtr p1, const PersonPtr p2) {
  /*
    ...
    A value greater than zero indicates that 
   ...
   */
    ...
    return strcmp(p1->name, p2->name) < 0; // true if alphabetical order
}
...
//void List_PersonPtr_Iterator_delet(List_PersonPtr_Iterator *it);
...
List_PersonPtr_Iterator List_PersonPtr_Iterator_new(Link* link){
  ...
  it.deref = &deref;
  it.dec = &dec;
  //it.erase = &List_PersonPtr_Iterator_delet;
  return it;
}
...
void List_PersonPtr_Iterator_delet(List_PersonPtr_Iterator it)
{
  List_PersonPtr_Iterator* iter = &it;
  free(iter->link);
  free(iter);
}
*/
...
struct Link* SortedMerge(struct Link* a, struct Link* b)
{
  ...
  if ( 
        PersonPtr_less(         // returns true if alphabetical order
                        ((Node*)(a))->p, // e.g. jane
                        ((Node*)(b))->p  // e.g. mike
                      )
     )
