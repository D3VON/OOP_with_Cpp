...
void Student_delet(Person* p)
{
  // IF STATEMENTS ARE ***ANATHEMA*** TO GOOD OO DESIGN!!!
  // very bad to do: if(str == "something")
  if ( strcmp(p->type_name, "Under") == 0 )
  { 
    // do nothing
  }
  else if ( strcmp(p->type_name, "Grad") == 0 )
  {    
  ...
  free((void *)p->name);
  p->name = 0;
  //free((char*)p->type_name);
  //p->type_name = 0;
  free(p);
  p = 0;
}

void Professor_delet(Person* p)
{  
  // Professor specific
	free((void *)((Professor *)p)->office);
	((Professor *)p)->office = 0;
  // person specific
	free((void *)p->name);
	p->name = 0;	
  //free((char*)p->type_name);
